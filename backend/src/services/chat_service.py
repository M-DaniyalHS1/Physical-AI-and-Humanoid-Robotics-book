    ...
    def process_user_message(self, db: Session, session_id: str, user_message: str) -> str:
        ...
        if session.mode == "selected-text-only" and session.selected_text:
            system_prompt += f"\n\nIMPORTANT: The user has selected specific text: '{session.selected_text}'. " \
                            ...
                            "Only use information from the selected text and related concepts. " \
                            "Do not provide general information outside of the selected text unless absolutely necessary."
